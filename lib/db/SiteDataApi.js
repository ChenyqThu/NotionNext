...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // 工具函数：查找文章（兼容 slug normalize + href 匹配 + UUID fallback）
  const findPost = () => {
    if (!props?.allPages) return null
    const normalizedFullSlug = normalizeSlugForMatch(fullSlug)
    return (
      // 1. 完整 slug 匹配（支持尾部 /、.html、大小写差异及 href 匹配）
      props.allPages.find(p => {
        if (!p || p?.type?.includes('Menu')) return false
        return (
          p.slug === fullSlug ||
          normalizeSlugForMatch(p.slug) === normalizedFullSlug ||
          normalizeSlugForMatch(p.href) === normalizedFullSlug
        )
      }) ||
      // 2. UUID 匹配
...
function normalizeSlugForMatch(path = '') {
...
function getCustomNav({ allPages }) {
  ...
  if (allPages && allPages.length > 0) {
    allPages.forEach(p => {
      const href = p.href || p.to || (p.slug ? `/${p.slug}` : '')
      customNav.push({
        icon: p.icon || null,
        name: p.title || p.name || '',
        href,
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  if (menuPages && menuPages.length > 0) {
    menuPages.forEach(e => {
      const href = e.href || e.to || (e.slug ? `/${e.slug}` : '')
      e.href = href
      e.to = href
      e.show = true
      if (e.type === 'Menu') {

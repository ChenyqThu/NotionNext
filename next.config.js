  ...
  webpack: (config, { dev, isServer }) => {
    ...
    if (!process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY) {
      config.resolve.alias['@clerk/nextjs$'] = path.resolve(
        __dirname,
        'lib/stubs/clerk-nextjs.js'
      )
      config.resolve.alias['@clerk/nextjs/server$'] = path.resolve(
        __dirname,
        'lib/stubs/clerk-nextjs-server.js'
      )
      config.resolve.alias['@clerk/localizations$'] = path.resolve(
        __dirname,
        'lib/stubs/clerk-localizations.js'
      )
    }
    ...
    if (!isServer) {
      console.log('[默认主题]', path.resolve(__dirname, 'themes', BLOG.THEME))
    }
